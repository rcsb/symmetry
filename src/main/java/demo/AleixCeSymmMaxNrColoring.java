	...
	public static void main(String[] args){
		...
		try {
			...
			while (i<order) {
				...
				System.out.println(afpChain.getPdbAln()[0][0][0]);
				afpAlignments[i-1] = afpChain;
				i++;
			}
			//Get the residue numbers (start and end) for each subunit
			String afpResidues = "";
			int n = afpAlignments[0].getPdbAln()[0][0].length;
			for (int k=0; k<n; k++){
				afpResidues += afpAlignments[0].getPdbAln()[0][0][k];
				System.out.println(afpResidues);
			}
			/*
