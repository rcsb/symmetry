...
public class SubunitMapper {
	...
	private List<Integer> getMappingByStructuralSuperposition() {
		...
		GlobalSequenceGrouper seqGroup1 = new GlobalSequenceGrouper(structure1, MIN_SEQUENCE_LENGTH);
		List<Atom[]> cas1 = seqGroup1.getCalphaTraces();
		GlobalSequenceGrouper seqGroup2 = new GlobalSequenceGrouper(structure2, MIN_SEQUENCE_LENGTH);
...
//		vectors[2] = null;
	...
	private FindQuarternarySymmetry getQuaternarySymmetry(Structure structure) {
		GlobalSequenceGrouper seqGroup = new GlobalSequenceGrouper(structure, MIN_SEQUENCE_LENGTH);
		...
		System.out.println("Chain ids in cluster order: " + seqGroup.getOrderedChainIDList());
		return finder;
	}
